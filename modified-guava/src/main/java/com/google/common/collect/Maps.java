...
public final class Maps {
	 ...
	 *
	 * <p>
	 * <b>Note:</b> if {@code K} is an {@link Enum} type, use {@link #newEnumMap}
	 * instead.
	 *
	 ...
	 *
	 * <p>
	 * <b>Warning</b>: If the function rejects {@code null}, caution is required to
	 * make sure the set does not contain {@code null}, because the view cannot stop
	 * {@code null} from being added to the set.
	 *
	 * <p>
	 * <b>Warning:</b> This method assumes that for any instance {@code k} of key
	 ...
	 * they typically contain strings, which is awkward. This method lets you get a
	 * plain-old-{@code Map} out of a {@code Properties}.
	 *
	 ...
	 * {@link Entry#setValue} operation throws an
	 * {@link UnsupportedOperationException}, as do any operations that would modify
	 * the returned set.
	 *
	...
	private static final class BiMapConverter<A, B> extends Converter<A, B> implements Serializable {
		...
		// There's really no good way to implement toString() without printing the
		// entire BiMap, right?
		@Override
		public String toString() {
	...
	/**
	 * Returns a synchronized (thread-safe) bimap backed by the specified bimap. In
	 * order to guarantee serial access, it is critical that <b>all</b> access to
	 * the backing bimap is accomplished through the returned bimap.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Changes in the underlying map are reflected in this view. Conversely, this
	 * view supports removal operations, and these are reflected in the underlying
	 * map.
	 *
	 * <p>
	 * It's acceptable for the underlying map to contain null keys, and even null
	 * values provided that the function is capable of accepting null input. The
	 * transformed map might contain null values, if the function sometimes gives a
	 * null result.
	 *
	 * <p>
	 * The returned map is not thread-safe or serializable, even if the underlying
	 * map is.
	 *
	 * <p>
	 * The function is applied lazily, invoked when needed. This is necessary for
	 * the returned map to be a view, but it means that the function will be applied
	 * many times for bulk operations like {@link Map#containsValue} and
	 * {@code Map.toString()}. For this to perform well, {@code function} should be
	 * fast. To avoid lazy evaluation when the returned map doesn't need to be a
	 ...
	 * provided that the transformer is capable of accepting null inputs. The
	 * transformed map might contain null values if the transformer sometimes gives
	 * a null result.
	 *
	 * <p>
	 * The returned map is not thread-safe or serializable, even if the underlying
	 ...
	 * the returned map to be a view, but it means that the transformer will be
	 * applied many times for bulk operations like {@link Map#containsValue} and
	 * {@link Object#toString}. For this to perform well, {@code transformer} should
	 * be fast. To avoid lazy evaluation when the returned map doesn't need to be a
	 * view, copy the returned map into a new map of your choosing.
	 *
	 * <p>
	 ...
	 * sorted map's entries. In contrast to {@link #transformValues}, this method's
	 * entry-transformation logic may depend on the key as well as the value.
	 *
	 * <p>
	 * All other properties of the transformed map, such as iteration order, are
	 ...
	 * navigable map's entries. In contrast to {@link #transformValues}, this
	 ...
	 * the filtered map or its views, only mappings whose keys satisfy the filter
	 ...
	 *
	 * <p>
	 * Many of the filtered map's methods, such as {@code size()}, iterate across
	 * every key/value mapping in the underlying map and determine which satisfy the
	 ...
	 * satisfy a predicate. The returned map is a live view of {@code
	 ...
	 * satisfy a predicate. The returned bimap is a live view of {@code unfiltered};
	 * changes to one affect the other.
	 *
	 * <p>
	 * The resulting bimap's {@code keySet()}, {@code entrySet()}, and
	 * {@code values()} views have iterators that don't support {@code remove()},
	 * but all other methods are supported by the bimap and its views. When given a
	 * key that doesn't satisfy the predicate, the bimap's {@code
	 * put()}, {@code forcePut()} and {@code putAll()} methods throw an
	 * {@link IllegalArgumentException}.
	 *
	 * <p>
	 * When methods such as {@code removeAll()} and {@code clear()} are called on
	 * the filtered bimap or its views, only mappings that satisfy the filter will
	 * be removed from the underlying bimap.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Many of the filtered bimap's methods, such as {@code size()}, iterate across
	 * every key in the underlying bimap and determine which satisfy the filter.
	 * When a live view is <i>not</i> needed, it may be faster to copy the filtered
	 * bimap and use the copy.
	 *
	 * <p>
	 ...
	 * The resulting map's {@code keySet()}, {@code entrySet()}, and {@code
	 * values()} views have iterators that don't support {@code remove()}, but all
	 * other methods are supported by the map and its views. When given a value that
	 * doesn't satisfy the predicate, the map's {@code put()}, {@code
	 * putAll()}, and {@link Entry#setValue} methods throw an
	 * {@link IllegalArgumentException}.
	 *
	 * <p>
	 * When methods such as {@code removeAll()} and {@code clear()} are called on
	 * the filtered map or its views, only mappings whose values satisfy the filter
	 * will be removed from the underlying map.
	 *
	 * <p>
	 * The returned map isn't threadsafe or serializable, even if {@code
	...
	/**
	 * Returns a bimap containing the mappings in {@code unfiltered} whose values
	 * satisfy a predicate. The returned bimap is a live view of {@code unfiltered};
	 * changes to one affect the other.
	 *
	 * <p>
	 ...
	 * {@code values()} views have iterators that don't support {@code remove()},
	 * but all other methods are supported by the bimap and its views. When given a
	 * value that doesn't satisfy the predicate, the bimap's {@code put()},
	 * {@code forcePut()} and {@code putAll()} methods throw an
	 * {@link IllegalArgumentException}. Similarly, the map's entries have a
	 * {@link Entry#setValue} method that throws an {@link IllegalArgumentException}
	 ...
	 * <p>
	 * Many of the filtered bimap's methods, such as {@code size()}, iterate across
	 * every value in the underlying bimap and determine which satisfy the filter.
	 * When a live view is <i>not</i> needed, it may be faster to copy the filtered
	 * bimap and use the copy.
	 *
	 * <p>
	 ...
	 * predicate. The returned map is a live view of {@code unfiltered}; changes to
	 * one affect the other.
	 *
	 * <p>
	 * The resulting map's {@code keySet()}, {@code entrySet()}, and {@code
	 * values()} views have iterators that don't support {@code remove()}, but all
	 * other methods are supported by the map and its views. When given a key/value
	 * pair that doesn't satisfy the predicate, the map's {@code put()} and
	 * {@code putAll()} methods throw an {@link IllegalArgumentException}.
	 * Similarly, the map's entries have a {@link Entry#setValue} method that throws
	 * an {@link IllegalArgumentException} when the existing key and the provided
	 * value don't satisfy the predicate.
	 *
	 * <p>
	 * When methods such as {@code removeAll()} and {@code clear()} are called on
	 * the filtered map or its views, only mappings that satisfy the filter will be
	 ...
	 * predicate. The returned bimap is a live view of {@code unfiltered}; changes
	 ...
	 * {@code values()} views have iterators that don't support {@code remove()},
	 * but all other methods are supported by the bimap and its views. When given a
	 * key/value pair that doesn't satisfy the predicate, the bimap's {@code put()},
	 * {@code forcePut()} and {@code putAll()} methods throw an
	 * {@link IllegalArgumentException}. Similarly, the map's entries have an
	 * {@link Entry#setValue} method that throws an {@link IllegalArgumentException}
	 * when the existing key and the provided value don't satisfy the predicate.
	 *
	 * <p>
	 * When methods such as {@code removeAll()} and {@code clear()} are called on
	 * the filtered bimap or its views, only mappings that satisfy the filter will
	 ...
	 * <p>
	 * Many of the filtered bimap's methods, such as {@code size()}, iterate across
	 * every key/value mapping in the underlying bimap and determine which satisfy
	...
	static class FilteredEntryMap<K, V> extends AbstractFilteredMap<K, V> {
		/**
		 * Entries in this set satisfy the predicate, but they don't validate the input
		 * to {@code Entry.setValue()}.
		 */
	...
	/**
	 * Returns an unmodifiable view of the specified navigable map. Query operations
	 * on the returned map read through to the specified map, and attempts to modify
	 * the returned map, whether direct or via its views, result in an
	 * {@code UnsupportedOperationException}.
	 *
	 * <p>
	 * The returned navigable map will be serializable if the specified navigable
	 * map is serializable.
	 *
	 ...
	 * <b>all</b> access to the backing navigable map is accomplished through the
	 * returned navigable map (or its views).
	 *
	 * <p>
	 * It is imperative that the user manually synchronize on the returned navigable
	...
	abstract static class ImprovedAbstractMap<K, V> extends AbstractMap<K, V> {
		 ...
		 * invoked at most once on a given map, at the time when {@code entrySet} is
		 * first called.
		 */
	 ...
	 * map entries. If {@code o} is an instance of {@code Map.Entry}, it is wrapped
	 * using {@link #unmodifiableEntry} to protect against a possible nefarious
	 * equals method.
	 *
	 * <p>
	 ...
	 * entries. If {@code o} is an instance of {@code Map.Entry}, it is wrapped
	 * using {@link #unmodifiableEntry} to protect against a possible nefarious
	 * equals method.
	 *
	 * <p>
